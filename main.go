...
func mainWithExitCode() exitCode {
	...
	// llm-prof: stop sampling after the requested duration.
	if cfg.Duration > 0 {
		log.Infof("Sampling for %s", cfg.Duration)
		go func() {
			select {
	...
	log.Info("Exiting ...")
	if err := rep.WriteOutput(); err != nil {
		log.Errorf("Failed to write profile output: %v", err)
		return exitFailure
	}
	log.Infof("Wrote profile to %s", cfg.OutputPath)
	return exitSuccess
}
