...
所有对比图统一流程：py-spy 与 llm-prof 都先输出 **pprof**（llm-prof 直接 `-o .pb.gz`，py-spy raw 经 `pyraw2pprof` 转换），再经 `demo-cases/pprof2folded.py` + **flamegraph.pl** 统一渲染成标准 SVG 火焰图（`demo-cases/gen_unified_svgs.sh`），两侧渲染风格完全一致。
...
### 开销-采样率曲线（10Hz → 10000Hz）单线程 CPU 密集负载（`case_hotspot.py`，12s 采样，各 2 轮中位，`demo-cases/run_rate_curve.sh` 可复现）：

| 采样率 | llm-prof（每 CPU） | py-spy（每进程） |
|---|---|---|
| 10Hz | +1.2% | +18.3% |
| 100Hz | +2.2% | +33.7% |
| 1000Hz | +3.2% | **+56.9%** |
...
![开销-采样率曲线](demo-cases/rate_curve.svg)

**结论**：eBPF 中断 + pid 过滤的开销极低——llm-prof 从 10Hz 到 **10000Hz**（单线程负载实际中断率 = 名义值）开销始终在 **+1~3.5%** 区间，采样率对性能影响几乎可忽略；py-spy 的 ptrace 暂停有较高固定成本（10Hz 即 +18%），且随采样率显著上升（10000Hz 达 +81%）。

### 开销与栈深的关系（中断 vs unwind 分解）

开销确实与应用本身（栈深）有关。用递归负载（`demo-cases/stack_bench.py`，5 档深度）在 10000Hz 下分解成本：
"中断+过滤基线"用 `-pid` 指向空闲进程（eBPF 只过滤不 unwind），与"完整 unwind"（`-pid` 指向目标）的差值即 unwind 成本
（`demo-cases/run_stack_exp.sh` 可复现）：

| 递归深度 | 实际帧深 | 中断+过滤 | unwind 成本 | 总开销 |
|---|---|---|---|---|
| 5 | 16.4 | +2.6% | +0.7% | +3.3% |
| 20 | 23.0 | +1.8% | +2.8% | +4.6% |
| 50 | 36.5 | -0.8%* | +1.4% | +0.6%* |
| 100 | 54.4 | +2.3% | +2.4% | +4.7% |
| 200 | 95.2 | +1.5% | +5.9% | +7.4% |
...
火焰图对比（GIL 争用 case，左侧 py-spy 全显示计算、右侧 llm-prof off-cpu 揭示 89% 等锁；两图均为 pprof → flamegraph.pl 统一渲染）：
