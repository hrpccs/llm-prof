...
// (off-cpu profiling). At load time loadProbeUnwinders repoints its
// get_per_cpu_record() from per_cpu_records to this map, so a perf sampler
// interrupting an in-flight kprobe unwind cannot clobber its record.
